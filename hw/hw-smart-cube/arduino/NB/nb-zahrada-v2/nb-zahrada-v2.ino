...
DHT dht(dhtPin, DHTTYPE);//teplomer nb vonku 
...
void loop () {  
  ...
  if (millis() > timer) {
    ...
    if(!networkUsed){
      ...
      sprintf (buff, "?action=record&val1=%d&type1=6&val2=%d&type2=11", (int)tempVal, (int)humidityVal);
      //Serial.println(buff);      
  ...
  if (millis() > timerSwitch) {
    ...
    if(!networkUsed){
      ...
      ether.browseUrl(PSTR("/index.php"), "?action=getswitches&id1=7&id2=16&id3=17", website, callback);
    }else{
...
static void callback (byte status, word off, word len) {
  ...
  if(val.indexOf("pastwo")>-1){
    ...
    if(values.substring(0,1).indexOf("1") == 0){
      Serial.println(F("ON switchCam1"));
      digitalWrite(switchCam1, LOW);
    } else {
      digitalWrite(switchCam1, HIGH);
      Serial.println(F("OFF switchCam1"));
    }   
    
    //switchCam2
    if(values.substring(1,2).indexOf("1") == 0){
      Serial.println(F("ON switchCam2"));
      digitalWrite(switchCam2, LOW);
    } else {
      digitalWrite(switchCam2, HIGH);
      Serial.println(F("OFF switchCam2"));
    }
